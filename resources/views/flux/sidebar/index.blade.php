...
@php
    $classes = Flux::classes('[grid-area:sidebar]')->add('z-1 flex flex-col gap-4 [:where(&)]:w-64 p-4');
    ...
    if ($stashable) {
        $attributes = $attributes
            ->merge([
                ...
                'x-init' =>
                    '$el.classList.add(\'-translate-x-full\', \'rtl:translate-x-full\'); $el.removeAttribute(\'data-mobile-cloak\'); $el.classList.add(\'transition-transform\')',
            ])
            ->class([
